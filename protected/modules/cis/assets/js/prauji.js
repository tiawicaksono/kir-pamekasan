...
function getInformationPrauji() {
  ...
  $("#id_kendaraan_prauji").val(id_kendaraan);
  $("#no_antrian_prauji").val(no_antrian);
  $.ajax({
    type: "POST",
    url: "Prauji/LoadImage",
      ...
      function (r) {
        if (r) {
          ...
          $.ajax({
            ...
            success: function (data) {
              hidelargeloader();
              saveFormPrauji(urlAct, id_hasil_uji, kirim, posisi, username);
              $("#formUpload").each(function () {
                this.reset();
              });
              $("#replace_file").load("Pengujian/ReplaceFile");
              return false;
            },
            error: function () {
              hidelargeloader();
...
function saveFormPrauji(urlAct, id_hasil_uji, kirim, posisi, username) {
  $.ajax({
    type: "POST",
    url: urlAct,
    data: {
      idhasil: id_hasil_uji,
      variabel: kirim,
      posisi: posisi,
      username: username,
    },
    success: function (data) {
      ...
      $("#img_depan_emisi").attr("src", "data:image/jpeg;base64," + "");
      $("#img_belakang_emisi").attr("src", "data:image/jpeg;base64," + "");
      $("#img_kiri_emisi").attr("src", "data:image/jpeg;base64," + "");
      $("#img_kanan_emisi").attr("src", "data:image/jpeg;base64," + "");
      prosesSearchPrauji();
      prosesSearchEmisi();
    },
    error: function () {
...
function captureCamera() {
  var id_hasil_uji = $("#id_hasil_uji_prauji").val();
  var id_kendaraan = $("#id_kendaraan_prauji").val();
  // alert($("#jnsbody").val());
  if (id_hasil_uji != "") {
    $.ajax({
