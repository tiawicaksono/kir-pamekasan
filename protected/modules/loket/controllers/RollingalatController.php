...
//    }
    ...
    private function pengujiCis1() {
        ...
        $criteriacis->addInCondition('user_status',array(1));
        $criteriacis->addCondition("position_id = 7");
        $criteriacis->addCondition("alat_uji = 'CIS 1'");
        $criteriacis->order = 'user_name ASC';
    ...
    private function pengujiCis2() {
        ...
        $criteriacis->addInCondition('user_status',array(1));
        $criteriacis->addCondition("position_id = '7'");
        $criteriacis->addCondition("alat_uji = 'CIS 2'");
        $criteriacis->order = 'user_name ASC';
        $pengujiCis = TblUser::model()->findAll($criteriacis);
        return $pengujiCis;
    }
    ...
    public function actionProsesRolling() {
        $updates = "UPDATE tbl_user SET pitlift = 'false', rem = 'false', lampu = 'false', prauji = 'false', emisi = 'false' WHERE position_id = '7'";
        ...
        if (isset($_POST['brake1'])) {
            ...
            foreach (array_keys($brake1) as $brk1):
                $update = "UPDATE tbl_user SET rem = 'true' WHERE id_user = " . $brk1;
                Yii::app()->db->createCommand($update)->query();
            endforeach;
        }
        if (isset($_POST['brake2'])) {
            ...
            foreach (array_keys($brake2) as $brk2):
                $update = "UPDATE tbl_user SET rem = 'true' WHERE id_user = " . $brk2;
                Yii::app()->db->createCommand($update)->query();
            endforeach;
        }
        ...
        if (isset($_POST['headlight1'])) {
            ...
            foreach(array_keys($headlight1) as $lamp1):
                $update = "UPDATE tbl_user SET lampu = 'true' WHERE id_user = " . $lamp1;
                Yii::app()->db->createCommand($update)->query();
            endforeach;
        }
        if (isset($_POST['headlight2'])) {
            ...
            foreach(array_keys($headlight2) as $lamp2):
                $update = "UPDATE tbl_user SET lampu = 'true' WHERE id_user = " . $lamp2;
                Yii::app()->db->createCommand($update)->query();
            endforeach;
        }
...
//        print_r($_POST);
        ...
        foreach(array_keys($checkbox) as $check):
            $update = "UPDATE tbl_user SET alat_uji = '" . $cis . "' WHERE id_user = " . $check;
            Yii::app()->db->createCommand($update)->query();
        endforeach;
    }
    
    public function actionProsesSwitchAll() {
        $update1 = "UPDATE tbl_user SET alat_uji = 'CIS 3' WHERE position_id = 7 AND alat_uji = 'CIS 1'";
        Yii::app()->db->createCommand($update1)->query();
        
        $update2 = "UPDATE tbl_user SET alat_uji = 'CIS 1' WHERE position_id = 7 AND alat_uji = 'CIS 2'";
        Yii::app()->db->createCommand($update2)->query();
        
        $update3 = "UPDATE tbl_user SET alat_uji = 'CIS 2' WHERE position_id = 7 AND alat_uji = 'CIS 3'";
