...
class KedatanganController extends Controller
{
    ...
    public function actionProsesDatang()
    {
        ...
        $criteria->addCondition("img1 <> '' OR img2 <> ''");
        $criteria->order = 'id_hasil_uji DESC';
        $dataHasilUji = TblHasilUji::model()->find($criteria);
        if (empty($dataHasilUji)) {
            $img1 = '';
            $img2 = '';
        } else {
            $img1 = $dataHasilUji->img1;
            $img2 = $dataHasilUji->img2;
        }

        $insert = "Select insert_hasil(" . $id_daftar . "," . $id_uji . "," . $id_kendaraan . ",'Auto','0','" . $img1 . "','" . $img2 . "')";
    ...
    public function actionGetListDataDatang()
    {
        ...
        $result = VDatangTgl::model()->findAll($criteria);
        $dataJson = array();
        ...
        echo CJSON::encode(
            array(
                'total' => VDatangTgl::model()->count($criteria),
                'rows' => $dataJson,
            )
        );
        Yii::app()->end();
    ...
    public function actionReportKedatanganExcel()
    {
        ...
        header('Content-Disposition: attachment;filename="Kedatangan[' . $tgl_sekarang . '].xls"');
