...
class EditbukuController extends Controller
{
    ...
    //
    //    public function actionChacha() {
    //        $sql = "select kecamatan,id_kota from tbl_kendaraan where kecamatan <> '' OR kecamatan is not null";
    //        $data = Yii::app()->db->createCommand($sql)->queryAll();
    ////        print_r($data);
    //        foreach ($data as $dt) {
    //            if (!empty($dt['kecamatan'])) {
    //                $kecamatan = $dt['kecamatan'];
    //                $kota = $dt['id_kota'];
    //                $a = "SELECT id_kecamatan, nama FROM m_kecamatan WHERE nama = '$kecamatan' AND id_kota = '$kota'";
    //                $b = Yii::app()->db->createCommand($a)->queryRow();
    //
    //                $id_kecamatan = $b['id_kecamatan'];
    //                $nama_kecamatan = $b['nama'];
    //
    //                $insert = "UPDATE tbl_kendaraan SET kecamatan = '$nama_kecamatan', id_kecamatan = '$id_kecamatan' WHERE kecamatan = '$kecamatan'";
    ...
    public function actionProsesSearch()
    {
        ...
        if (empty($tbl_kendaran)) {
            ...
            $data['alamat_pemilik'] = $data_kendaraan->alamat;
            $data['tempat_lahir'] = $data_kendaraan->tmp_lahir;
            //            ECHO date("d/m/Y", strtotime($data_kendaraan->tgl_lahir));
            if ((date("d/m/Y", strtotime($data_kendaraan->tgl_lahir)) != '01/01/1970') and (date("d/m/Y", strtotime($data_kendaraan->tgl_lahir)) != '30/12/1899') and (date("d/m/Y", strtotime($data_kendaraan->tgl_lahir)) != '30/12/2018')) {
                $data['tgl_lahir'] = date("d/m/Y", strtotime($data_kendaraan->tgl_lahir));
                //                echo 'asd';
            } else {
                $data['tgl_lahir'] = '';
                //                echo 'adas';
            ...
            $data['kota'] = empty($data_kendaraan->id_kota) ? '3528' : $data_kendaraan->id_kota;
    ...
    public function actionSaveForm()
    {
        ...
        $var_no_uji = str_replace("'", " ", strtoupper($_POST['nomer_uji']));
        $var_nomor_uji = preg_replace("/([[:alpha:]])([[:digit:]])/", "\\1 \\2", $var_no_uji);
        $nomer_uji = preg_replace("/([[:digit:]])([[:alpha:]])/", "\\1 \\2", $var_nomor_uji);
        $nomer_uji = $var_no_uji;
        $var_no_kendaraan = str_replace("'", " ", strtoupper($_POST['nomer_kendaraan']));
        $var_nomor_kendaraan = preg_replace("/([[:alpha:]])([[:digit:]])/", "\\1 \\2", $var_no_kendaraan);
        $nomer_kendaraan = preg_replace("/([[:digit:]])([[:alpha:]])/", "\\1 \\2", $var_nomor_kendaraan);

        //        $nomer_uji = str_replace("'", "`", strtoupper($_POST['nomer_uji']));
        ...
        if (!empty($birth_date)) {
            ...
            $tgl_lahir = '01/01/1970';
            //            echo 'dsa';
        }
        $kelamin = $_POST['kelamin'];
        //        $awal_pemakaian = date("m/d/Y", strtotime($_POST['awal_pemakaian']));
    ...
    public function actionSaveBaruForm()
    {
        ...
        $idKendaraan = 0;
        $var_no_uji = str_replace("'", " ", strtoupper($_POST['nomer_uji_baru']));
        $var_nomor_uji = preg_replace("/([[:alpha:]])([[:digit:]])/", "\\1 \\2", $var_no_uji);
        $nomer_uji = preg_replace("/([[:digit:]])([[:alpha:]])/", "\\1 \\2", $var_nomor_uji);
        $var_no_kendaraan = str_replace("'", " ", strtoupper($_POST['nomer_kendaraan_baru']));
        $var_nomor_kendaraan = preg_replace("/([[:alpha:]])([[:digit:]])/", "\\1 \\2", $var_no_kendaraan);
        $nomer_kendaraan = preg_replace("/([[:digit:]])([[:alpha:]])/", "\\1 \\2", $var_nomor_kendaraan);
        ...
        //        $tgl_lahir = date("m/d/Y", strtotime($_POST['tgl_lahir']));
