...
class TblRetribusi extends CActiveRecord {
    ...
    public function totalRetribusiPerHari($tgl) {
        $query = "SELECT (SUM(b_berkala)::numeric)+(SUM(b_pertama)::numeric)+(SUM(b_tlt_uji)::numeric)+(SUM(b_plat_uji)::numeric)"
                . "+(SUM(b_buku)::numeric)+(SUM(b_tnd_samping)::numeric)+(SUM(b_jbb)::numeric)+(SUM(b_rekom)::numeric) as total FROM tbl_retribusi WHERE "
//                . "tgl_retribusi = now()::date and validasi = true";
                . "tgl_retribusi = TO_DATE('" . $tgl . "', 'DD-Mon-YY') and validasi = true";
        return Yii::app()->db->createCommand($query)->queryRow();
    }
