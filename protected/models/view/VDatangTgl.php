 ...
 */
class VDatangTgl extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('no_uji, no_chasis, no_mesin', 'length', 'max'=>30),
			array('nama_pemilik', 'length', 'max'=>100),
			array('no_kendaraan', 'length', 'max'=>12),
			array('jns_kend', 'length', 'max'=>40),
			array('nm_uji', 'length', 'max'=>50),
			array('tgl_uji, no_berkas, id_kendaraan, id_daftar, id_retribusi, datang, id_uji, no_antrian, stts_no', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('tgl_uji, no_berkas, no_uji, nama_pemilik, no_kendaraan, no_chasis, no_mesin, id_kendaraan, id_daftar, id_retribusi, datang, jns_kend, nm_uji, id_uji, no_antrian, stts_no', 'safe', 'on'=>'search'),
		);
	}
